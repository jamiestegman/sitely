...
const Post = ({ contents, data }) => {
  ...
  const { author, title, description, tags, website, img } = data;
          ...
          <Article>
            <div className="title">
              ...
              <h1 id={convertToSlug(title)}>{title}</h1>
                <a className="live-site-link" href={website} target="_blank" rel="noreferrer"><BsBoxArrowUpRight /></a>
              </div>
              <p>{description}</p>
              <div className="tags">
                {tags.map(tag => (
                  <Link href={`/breakdowns?tag=${tag}`} className="tag" key={tag}>{`#${tag}`}</Link>
                ))}
              </div>
              <img src={img} />
            </div>
...
export const getStaticPaths = async () => {
  ...
  const files = fs.readdirSync('posts/breakdowns');
...
export const getStaticProps = async ({ params: { slug } }) => {
  ...
  const markdown = fs.readFileSync(path.join('posts/breakdowns', slug + '.mdx'), 'utf-8');
